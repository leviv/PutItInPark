...
def locations_paginated(page_number):
    try:
        locations = Location.query.paginate(
            page=page_number, per_page=12, error_out=False
        ).items
        location_schema = LocationSchema(many=True)
        return (
            locations,
            location_schema,
        )  # jsonify(location_schema.dump(locations)), 200
    except Exception as e:
        return "An Error Occured:" + str(e), 500
    ...
    try:
        if request.data:
            ...
            if direction == "asc":
                ...
                order = desc
            nationalparks = (
                Nationalparks.query.order_by(order(col))
                .paginate(page=1, per_page=15, error_out=False)
                .items
            )
        else:
            nationalparks = Nationalparks.query.paginate(
                page=1, per_page=49, error_out=False
            ).items
    ...
    try:
        if request.data:
            ...
            if direction == "asc":
                ...
                order = desc
            recreations = (
                Recreation.query.order_by(order(col))
                .paginate(page=1, per_page=15, error_out=False)
                .items
            )
        else:
            recreations = Recreation.query.paginate(
                page=1, per_page=210, error_out=False
            ).items
