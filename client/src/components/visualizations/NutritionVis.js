...
class NutritionVis extends React.Component {
    ...
    fetch(API_ENDPOINT)
      ...
      .then((data) => {
        ...
        data.data.forEach((label) => {
          ...
          this.state.carbohydrates += Math.max(
            label.attributes.carbohydrates,
            0
          ); //C
          this.state.cholesterol += Math.max(
            label.attributes.cholesterol / 1000,
            0
          ); //CH
  ...
  plotNutrition() {
    ...
    this.state.data.forEach((stat) => {
      ...
      } else data[stat.name] = stat.value;
    });
      ...
      return (
        <React.Fragment>
          ...
          <h3 className="text-center">
            <span>Nutrition Visualization</span>
          </h3>
          ...
          <div className="row">
            <div className="col-md-3">
              <div>
                <p>
                  <strong>Average nutrition stats</strong>
                </p>
                <p>
                  CA: Calcium:{" "}
                  {(this.state.calcium / this.state.numLabels).toFixed(2)}g
                </p>
                <p>
                  C: Carbohydrates:{" "}
                  {(this.state.carbohydrates / this.state.numLabels).toFixed(2)}
                  g
                </p>
                <p>
                  CH: Cholesterol:{" "}
                  {(this.state.cholesterol / this.state.numLabels).toFixed(2)}g
                </p>
                <p>
                  F: Fat: {(this.state.fat / this.state.numLabels).toFixed(2)}g
                </p>
                <p>
                  FI: Fiber:{" "}
                  {(this.state.fiber / this.state.numLabels).toFixed(2)}g
                </p>
                <p>
                  I: Fiber:{" "}
                  {(this.state.fiber / this.state.numLabels).toFixed(2)}g
                ...
                <p>
                  P: Protein:{" "}
                  {(this.state.protein / this.state.numLabels).toFixed(2)}g
                </p>
                <p>
                  SF: Saturated fat:{" "}
                  {(this.state.saturatedFat / this.state.numLabels).toFixed(2)}g
                </p>
                <p>
                  S: Sugars:{" "}
                  {(this.state.sugars / this.state.numLabels).toFixed(2)}g
                </p>
                <p>
                  SO: Sodium:{" "}
                  {(this.state.sodium / this.state.numLabels).toFixed(2)}g
                </p>
              </div>
            </div>
