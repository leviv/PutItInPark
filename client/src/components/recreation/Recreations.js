...
import { API_ENDPOINT, convertToRows, expandFilters } from '../helpers/Helpers.js'
...
class Recreations extends React.Component {
  ...
  makeApiCall(pageNumber) {
    fetch(endpoint + "?q="+ JSON.stringify(this.state.query) + "&results_per_page=12&page=" + pageNumber)
          ...
          <div className="collapsed" id="augment-container">
            ...
            <div className="row">
              <div className="col-md-8 model-filter">
                <h4>Filter by</h4>
                <select className="form-control" id="reserveFilter" onChange={this.applyFilters}>
                  <option selected>Reservable</option>
                  <option>Yes</option>
                  <option>No</option>
                </select>
                <select className="form-control" id="stayFilter" onChange={this.applyFilters}>
                  <option selected>Stay Limit</option>
                  <option>Yes</option>
                  <option>No</option>
                </select>
                <select className="form-control" id="activityFilter" onChange={this.applyFilters}>
